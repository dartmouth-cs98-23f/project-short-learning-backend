  ...
  try {
    ...
    let videoRecommendation: any = {
      ...
      videos: await Promise.all(
        recommendations.matches.map(async (match) => {
          ...
          return {
            ...
            topics: Array.isArray(match.metadata?.inferenceTopics)
              ? Array.from(match.metadata.inferenceTopics)
              : [],
            score: match.score,
            metadata
          }
...
async function getSearchVectors(videoId?: string, userId?: string) {
  ...
  try {
    ...
    if (videoId) {
      ...
      const history = await WatchHistory.getWatchHistories(
        { id: userId },
        { limit: 10, order: [['date', 'DESC']] }
      )

      const videoId =
        history.length > 0 ? history[0].videoId : '65d8fc1c95f306b28d1b887b' // TECHNIGALA: WARNING USES HARDCODING
      ...
      if (response.matches.length !== 1) {
        logger.error(
          `NO VIDEO "${videoId}" FOUND IN PINECONE FOR USER ${userId}`
        )
        throw new Error(
          `NO VIDEO "${videoId}" FOUND IN PINECONE FOR USER ${userId}`
        )
